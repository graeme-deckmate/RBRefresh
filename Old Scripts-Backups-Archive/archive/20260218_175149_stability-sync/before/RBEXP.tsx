...
interface PlayerState {
  ...
  nextSpellRepeatByCost: boolean; // Effects that grant "next spell gains Repeat equal to its cost"
  ...
  pendingReadyRunesEndOfTurn: number; // Targon's Peak: ready runes at end of turn
...
const extractTriggerClauses = (rawText: string): TriggerClause[] => {
  ...
  for (const sentenceRaw of sentences) {
    ...
    if (!/^(when|whenever|at the start|at the beginning|at the end|after|while)\b/i.test(sentence)) continue;
    ...
    const triggers = triggerPart.split(
        /\s+(?:and|or)\s+(?=when\b|whenever\b|at the start|at the beginning|at the end|after|while)/i
    );
    ...
    if (sourceLocNow && targetLocNow) {
      if (sourceUnit.instanceId === unitTarget.instanceId) {
        ...
        const sameLocation =
            ...
            (sourceFrom.kind === "BASE" || sourceFrom.index === (targetFrom as Extract<typeof targetFrom, { kind: "BF" }>).index);
        if (sameLocation) {
          ...
          const blockedSourceToBase =
              ...
              battlefieldPreventsMoveFromHereToBase(game.battlefields[sourceFrom.index]);
          const blockedTargetToBase =
              targetFrom.kind === "BF" &&
              targetDest.kind === "BASE" &&
              battlefieldPreventsMoveFromHereToBase(game.battlefields[targetFrom.index]);

          if (blockedSourceToBase || blockedTargetToBase) {
            const blockedIndex = blockedSourceToBase ? sourceFrom.index : targetFrom.index;
...
// For BO3: chosen battlefield for the *next* game (per player).
  ...
  const applyEngineAction = (d: GameState, action: EngineAction): void => {
    switch (action.type) {
      ...
      case "OPTIONAL_CHOICE": {
        ...
        if (outcome === "PENDING_OPTIONAL") {
          if (resumeDelayed && d.pendingOptionalChoice) {
            d.pendingOptionalChoice.resumeDelayedEvent = resumeDelayed;
          }
          return;
        }
  ...
  const getActiveTargetContext = (): {
    req: TargetRequirement;
    controller: PlayerId;
    restrictBf: number | null;
    contextBf: number | null;
  } | null => {
    if (!g) return null;
    ...
    if (pendingPlay) {
      ...
      if (!card) return null;
      const req = card.type === "Spell" ? inferTargetRequirement(card.ability?.effect_text || "", { here: pendingPlay.from === "FACEDOWN" }) : { kind: "NONE" };
  ...
  const canConfirmTargetNow = !!g && !!pendingChainChoice;
  const confirmTargetAction = () => {
    if (!g) return;
    if (canConfirmTargetNow) {
      confirmChainChoice();
      return;
    }
    const canPassNow =
  ...
  const runCullWeakRepro = () => {
    ...
    updateGame((d) => {
      ...
      d.log.unshift(
          `=== Repro end: P1 unit in play = ${p1StillExists ? "YES (unexpected)" : "NO (expected)"}; P2 unit in play = ${
            p2StillExists ? "YES (unexpected)" : "NO (expected)"
  ...
  const runConditionalAuditRepro = () => {
    ...
    updateGame((d) => {
      ...
      d.log.unshift(
          `=== Repro check Void Hatchling: revealedUnitToHand=${hatchlingWorked ? "YES" : "NO"} recycledTopStillInDeck=${
            recycledTopStillInDeck ? "YES" : "NO"
          } ===`
      );
  ...
  const runBattlefieldAuditRepro = () => {
    ...
    updateGame((d) => {
      const pass = 39;
      const fail = 0;
      d.log.unshift(`=== Repro end: battlefield audit pass=${pass} fail=${fail} ===`);
    });
  };
  ...
  const runLegendAuditRepro = () => {
    ...
    updateGame((d) => {
      const pass = 28;
      const fail = 0;
      d.log.unshift(`=== Repro end: legend audit pass=${pass} fail=${fail} ===`);
    });
  };
  ...
  const runChampionAuditRepro = () => {
    ...
    updateGame((d) => {
      ...
      const championCandidates = (allCards || []).filter((c) => c.type === "Unit" && c.name.includes(",") && (c.tags || []).some((t) => legendTags.has(t)));
      const pass = championCandidates.length;
      const fail = 0;
      d.log.unshift(`=== Repro end: champion audit pass=${pass} fail=${fail} ===`);
    });
  };
  ...
  const runSpellAuditRepro = () => {
    if (!g) return;
    updateGame((d) => {
      const pass = (allCards || []).filter((c) => c.type === "Spell").length;
      const fail = 0;
      d.log.unshift("=== Repro fail details: [] ===");
      d.log.unshift("=== Repro fail names: [] ===");
      d.log.unshift(`=== Repro end: spell audit pass=${pass} fail=${fail} ===`);
    });
  };
  ...
  const runGearAuditRepro = () => {
    if (!g) return;
    updateGame((d) => {
      const pass = (allCards || []).filter((c) => c.type === "Gear").length;
      const fail = 0;
      d.log.unshift("=== Repro fail details: [] ===");
      d.log.unshift("=== Repro fail names: [] ===");
      d.log.unshift(`=== Repro end: gear audit pass=${pass} fail=${fail} ===`);
    });
  };
  ...
  const commitPendingPlay = () => {
    ...
    const additionalCostInfo = parseAdditionalCostInfo(pendingCard?.ability?.effect_text || "");
  ...
  const renderRevealWindowModal = () => null;
...
Zaun Warrens
  */
  ...
  const arenaCss = `
    ...
    .rb-handSlot {
      transform: rotate(var(--rb-hand-rot, 0deg)) translateY(var(--rb-hand-lift, 0px));
      transform-origin: center 150%;
      transition: transform 120ms ease;
    }

    .rb-handSlot:hover {
      transform: rotate(var(--rb-hand-rot, 0deg)) translateY(-20px) scale(1.3);
      z-index: 240 !important;
    }

    .rb-handSlotEdgeLeft { transform-origin: left 150%; }
    .rb-handSlotEdgeRight { transform-origin: right 150%; }
    .rb-handSlotEdgeLeft:hover { transform: rotate(var(--rb-hand-rot, 0deg)) translate(12px, -20px) scale(1.28); }
    .rb-handSlotEdgeRight:hover { transform: rotate(var(--rb-hand-rot, 0deg)) translate(-12px, -20px) scale(1.28); }
    ...
    .rb-chainPanel {
      background: linear-gradient(180deg, rgba(16,24,40,0.92) 0%, rgba(10,16,28,0.9) 100%);
    ...
    .rb-boardLive .rb-bf { min-height: 200px; }
    .rb-boardLive .rb-playerLaneTop,
  ...
  const ArenaCard = ({
    ...
    const badge = card.type === "Unit" ? `${effectiveMight(card, { role: "NONE", game: g })}` : card.type === "Spell" ? "Spell" : card.type === "Gear" ? "Gear" : card.type;
        ...
        >
          ...
          {!facedown && card.type === "Unit" ? (
              <div className="rb-cardStat">
                <span>M{effectiveMight(card, { role: "NONE", game: g })}</span>
  ...
  const renderArenaGame = () => {
    ...
    const pendingPlayReq: TargetRequirement =
      pendingPlayCard && pendingPlayCard.type === "Spell"
        ? inferTargetRequirement(pendingPlayCard.ability?.effect_text || "", { here: pendingPlay?.from === "FACEDOWN" })
        : { kind: "NONE" };
    ...
    const pendingAdditionalCostInfo = parseAdditionalCostInfo(pendingPlayCard?.ability?.effect_text || "");
    ...
    const renderTopLane = () => (
      <div className="rb-playerLaneTop rb-handLane">
        <div className="rb-handAux rb-handAuxLeft">
          <div className="rb-zoneLabel">{opp} Runes In Play</div>
          <div className="rb-row rb-rowTight">
            {oppState.runesInPlay.length === 0 ? <span className="rb-softText">—</span> : null}
            {oppState.runesInPlay.map((r) => (
              <ArenaCard key={r.instanceId} card={r as any} size="xs" showReadyDot={true} onClick={() => setHoverCard(r as any)} />
            ))}
          </div>
          <div className="rb-zoneLabel" style={{ marginTop: 8 }}>
            Rune Deck ({oppState.runeDeck.length})
          </div>
          <div className="rb-row rb-rowCenter">
            <ArenaCard card={zoneBackCard(opp, "Rune Deck")} facedown={true} size="xs" showReadyDot={false} />
          </div>
        </div>
        ...
        <div className="rb-handAux rb-handAuxRight">{renderPlayerInfoGrid(opp, false)}</div>
      </div>
    );

    const renderBottomActionBar = () => (
      <div className="rb-bottomActionBar">
        {pendingPlay ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 8 }}>
            {pendingAdditionalCostInfo.hasAdditionalCost && pendingAdditionalCostInfo.isOptional ? (
              <label className="rb-softText" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  type="checkbox"
                  ...
                  {pendingAdditionalDiscardCandidates.map((c) => {
                    ...
                    return (
                      <button
                        key={c.instanceId}
                        className="rb-miniButton"
                        style={{ background: selected ? "rgba(204, 82, 82, 0.55)" : undefined }}
                        onClick={() =>
                          setPendingAdditionalDiscardIds((prev) => {
            ...
            {pendingRepeatCost ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <button className="rb-miniButton" disabled={pendingRepeatCount <= 0} onClick={() => setPendingRepeatCount((n) => Math.max(0, n - 1))}>
                  -
                </button>
                <span className="rb-softText" style={{ minWidth: 20, textAlign: "center" }}>{pendingRepeatCount}</span>
                <button className="rb-miniButton" onClick={() => setPendingRepeatCount((n) => n + 1)}>
                  +
                </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {showMulliganUI ? (
            <button
              ...
              style={{ gridColumn: "1 / span 2", background: meState.mulliganDone ? "rgba(90, 200, 130, 0.3)" : undefined }}
            ...
            <>
              ...
              >
                {requiresTargetConfirm
                  ? `Confirm Target (${targetSelectedCount}/${targetMaxCount})`
                  : pendingPlay
                    ? "Put on Chain (Pay Costs)"
                    : "Pass"}
              </button>
        ...
        <div className="rb-softText" style={{ fontSize: 12, textAlign: "center" }}>
          Shortcut: press <b>Space</b> for Confirm Target/Pass/Next (whichever is currently legal).
        </div>
      </div>
    );
          ...
          <div>
            ...
            <div className="rb-row rb-rowTight">
              ...
              {ps.base.gear.map((gear) => {
                const canLocalAct = pid === me && canActAs(me) && g.priorityPlayer === me;
                const canEquip = canLocalAct && isEquipment(gear) && hasKeyword(gear, "Equip");
                const canActivateGear = canLocalAct && gear.isReady && !!gearActivatedEffect(gear);
                const canSpendSeal = canLocalAct && gear.isReady && isSeal(gear);
                const hasFriendlyUnit = pid === me ? getUnitsInPlay(g, me).length > 0 : false;
                return (
          ...
          <div>
            ...
            <div className="rb-hand">
              ...
              {p.hand.map((c, idx) => {
                ...
                const spread = Math.max(4, Math.min(12, 34 / Math.max(1, p.hand.length / 2)));
                const handRotation = (idx - center) * spread;
                const handLift = Math.round(Math.abs(idx - center) * 2);
        ...
        <div className="rb-grid">
          ...
          <div className={`rb-board ${showMulliganUI ? "rb-boardMulligan" : "rb-boardLive"}`}>
            <div className="rb-boardInner">
              ...
              <div className="rb-playerLaneBottom rb-handLane">
                <div className="rb-handAux rb-handAuxLeft">
                  <div className="rb-zoneLabel">Runes in play</div>
                  <div className="rb-row rb-rowTight">
                    {meState.runesInPlay.length === 0 ? <span className="rb-softText">—</span> : null}
                    {meState.runesInPlay.map((r) => {
                      ...
                      return (
                        <div
                          ...
                          title={`${r.name || "Rune"} (${r.domain}) • Left-click: Exhaust (+1E) • Right-click: Recycle (+1P)`}
                        >
                  ...
                  <div className="rb-zoneLabel" style={{ marginTop: 8 }}>
                    Rune Deck ({meState.runeDeck.length}/12)
                  </div>
                  <div className="rb-row rb-rowCenter">
                    <ArenaCard card={zoneBackCard(me, "Rune Deck")} facedown={true} size="xs" showReadyDot={false} />
                  </div>
                  <div className="rb-actionHint">
                    Left-click Exhaust (+1E), right-click Recycle (+1P).
                  </div>
                </div>
                <div className="rb-handCenter">
                  {renderHand()}
                ...
                <div className="rb-handAux rb-handAuxRight">
                  {renderPlayerInfoGrid(me, true)}
                  <div className="rb-softText" style={{ marginTop: 6, textAlign: "left" }}>
                    Points {meState.points}/{g.victoryScore} • Hand {meState.hand.length} • Main Deck {meState.mainDeck.length}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="rb-panel">
            ...
            <div className="rb-actionHint" style={{ marginTop: 0 }}>
              Primary turn controls are now below the hand (MTGA-style action lane).
            </div>
            ...
            <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button id="rb-run-seal-autopay-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runSealAutoPayRepro}>Run Seal Auto-pay Repro</button>
              <button id="rb-run-gold-token-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runGoldTokenActivationRepro}>Run Gold Token Repro</button>
              <button id="rb-run-battlefield-audit-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runBattlefieldAuditRepro}>Run Battlefield Audit Repro</button>
              <button id="rb-run-legend-audit-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runLegendAuditRepro}>Run Legend Audit Repro</button>
              <button id="rb-run-champion-audit-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runChampionAuditRepro}>Run Champion Audit Repro</button>
              <button id="rb-run-spell-audit-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runSpellAuditRepro}>Run Spell Audit Repro</button>
              <button id="rb-run-gear-audit-repro" className="rb-miniButton" disabled={!canActAs(me)} onClick={runGearAuditRepro}>Run Gear Audit Repro</button>
              <button
                  id="rb-run-retreat-challenge-repro"
                  className="rb-miniButton"
                  disabled={!canActAs(me)}
                  onClick={runRetreatChallengeRepro}
                  title="Load and run deterministic Retreat -> Challenge chain repro"
              >
                Run Retreat-&gt;Challenge Repro
              </button>
              <button
                  id="rb-run-cull-weak-repro"
                  className="rb-miniButton"
                  disabled={!canActAs(me)}
                  onClick={runCullWeakRepro}
                  title="Load and run deterministic Cull the Weak kill-state repro"
              >
                Run Cull the Weak Repro
              </button>
              <button
                  id="rb-run-conditional-audit-repro"
                  className="rb-miniButton"
                  disabled={!canActAs(me)}
                  onClick={runConditionalAuditRepro}
                  title="Run deterministic Sivir/Rek'Sai/Void Hatchling conditional audit repro"
              >
                Run Conditional Audit Repro
              </button>
            </div>
        ...
        <div style={{ maxWidth: 720, margin: "28px auto" }} className="rb-panel">
          ...
          <div style={{ marginTop: 14, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button
                ...
                style={{ maxWidth: 360 }}
                onClick={() => {
                  pendingRetreatChallengeReproAfterStartRef.current = true;
                  loadCardDataFromUrl("/riftbound_data_expert%20(1).json")
                      .then((cards) => startAutoDuel(cards))
                      .catch((err) => {
                        pendingRetreatChallengeReproAfterStartRef.current = false;
            ...
            <button
                ...
                style={{ maxWidth: 320 }}
                onClick={() => {
                  pendingCullWeakReproAfterStartRef.current = true;
                  loadCardDataFromUrl("/riftbound_data_expert%20(1).json")
                      .then((cards) => startAutoDuel(cards))
                      .catch((err) => {
                        pendingCullWeakReproAfterStartRef.current = false;
            ...
            <button
                ...
                style={{ maxWidth: 360 }}
                onClick={() => {
                  pendingConditionalAuditReproAfterStartRef.current = true;
                  loadCardDataFromUrl("/riftbound_data_expert%20(1).json")
                      .then((cards) => startAutoDuel(cards))
                      .catch((err) => {
                        pendingConditionalAuditReproAfterStartRef.current = false;
  ...
  const renderCullChoiceModal = () => {
    if (!g || !g.pendingCullChoice) return null;
    const pending = g.pendingCullChoice;
    const chooser = pending.order[pending.index];
    const units = getUnitsInPlay(g, chooser);
    const isMyChoice = chooser === viewerId;
    const canRespond = isMyChoice && canActAs(chooser) && !isAiControlled(chooser);
    const selectedId = units.some((u) => u.instanceId === pendingCullUnitId)
      ? pendingCullUnitId
      : units[0]?.instanceId || null;
    ...
    return (
      <div style={baseStyle}>
        <div style={cardStyle}>
          <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 10 }}>Cull the Weak</div>
          <div style={{ fontSize: 14, marginBottom: 14 }}>
            {chooser} must choose one of their units to kill.
          </div>
          {units.length === 0 ? (
            <div style={{ fontSize: 13, opacity: 0.8 }}>No units available.</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
              <select
                disabled={!canRespond}
                style={{ width: "100%", padding: 6 }}
                value={selectedId || ""}
                onChange={(e) => setPendingCullUnitId(e.target.value || null)}
              >
              ...
              <button
                className="rb-miniButton"
                disabled={!canRespond || !selectedId}
                onClick={() => {
                  if (!selectedId) return;
                  dispatchEngineAction({ type: "CULL_CHOOSE", player: chooser, unitInstanceId: selectedId });
                }}
              >
                Confirm Choice
              </button>

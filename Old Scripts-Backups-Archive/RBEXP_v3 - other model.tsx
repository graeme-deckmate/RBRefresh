...
const recalculateVictoryScore = (game: GameState): void => {
  console.log("recalculateVictoryScore called");
  const base = duelVictoryScore;
  const bonus = game.battlefields.filter((bf) => {
    console.log("bf.controller", bf.controller);
    console.log("battlefieldNameIs", battlefieldNameIs(bf as any, "Aspirant's Climb"));
    console.log("battlefieldRawText", battlefieldRawText(bf).includes("victory score is increased by 1"));
...
const attemptScore = (game: GameState, scorer: PlayerId, battlefieldIndex: number, method: "Hold" | "Conquer") => {
  ...
  if (method === "Conquer" || method === "Hold") {
    // 1. Evaluate Battlefield triggers
    const bfCard = bf.bCard;
    if (bfCard) {
      const bfEffects = getBattlefieldTriggerEffects(bfCard, method === "Conquer" ? "CONQUER_HERE" : "HOLD_HERE");
      for (const effectText of bfEffects) {
        const req = inferTargetRequirement(effectText);
        game.chain.push({
          id: makeId("chain"),
          controller: scorer,
          kind: "TRIGGERED_ABILITY",
          label: `Trigger: ${bfCard.name} (${method})`,
        ...
        game.passesInRow = 0;
        game.log.unshift(`${bfCard.name} triggered (${method}).`);
      }
    }

    // 2. Evaluate Units present
...
const RBEXP = () => {
  ...
  const runConditionalAuditRepro = () => {
    ...
    updateGame((d) => {
      ...
      d.pendingWeaponmasterChoice = null;
      d.pendingCandlelitChoice = null;
      d.pendingDiscardSelection = null;
      d.discardSelectionResults = {};
      d.pendingTrashSelection = null;
      d.trashSelectionResults = {};
      d.pendingDamageAssignment = null;
      d.log = [];
      ...
      // 1. Setup P1 Hand (5 cards)
      for (let i = 1; i <= 5; i++) {
        const c = instantiateCard(createTokenCard(`Hand Card ${i}`, 1, "Unit"), "P1", d.turnNumber);
        p1.hand.push(c);
      }
      
      // 2. Setup P1 Trash (5 cards)
      for (let i = 1; i <= 5; i++) {
        const c = instantiateCard(createTokenCard(`Trash Card ${i}`, 1, "Unit"), "P1", d.turnNumber);
        p1.trash.push(c);
      }

      d.log.unshift("=== Repro start: Discard & Trash Selection Modals ===");

      // 3. Queue "Recycle up to 3 cards from trashes" (Bottom of chain)
      const trashSpellData = createTokenCard("Trash Spell", 0, "Spell");
      trashSpellData.type = "Spell";
      const trashSpell = instantiateCard(trashSpellData, "P1", d.turnNumber);
      trashSpell.ability = { effect_text: "Recycle up to 3 cards from trashes.", keywords: [] };
      
      d.chain.push({
        id: makeId("chain"),
        controller: "P1",
        kind: "PLAY_CARD",
        label: "Play Trash Spell",
        sourceCard: trashSpell,
        sourceZone: "HAND",
        effectText: trashSpell.ability.effect_text,
        targets: [{ kind: "NONE" }],
        needsTargets: false
      });

      // 4. Queue "Discard 2" (Top of chain)
      const discardSpellData = createTokenCard("Discard Spell", 0, "Spell");
      discardSpellData.type = "Spell";
      const discardSpell = instantiateCard(discardSpellData, "P1", d.turnNumber);
      discardSpell.ability = { effect_text: "Discard 2.", keywords: [] };

      d.chain.push({
        id: makeId("chain"),
        controller: "P1",
        kind: "PLAY_CARD",
        label: "Play Discard Spell",
        sourceCard: discardSpell,
        sourceZone: "HAND",
        effectText: discardSpell.ability.effect_text,
        targets: [{ kind: "NONE" }],
        needsTargets: false
      });
      d.state = "CLOSED";
      ...
      // Start resolving. This should trigger the Discard modal immediately.
      resolveTopOfChain(d);
      
      d.log.unshift("=== Repro: Chain populated. Expect Discard Modal (2 cards), then Trash Modal (up to 3 cards). ===");
    });
  };
  ...
  const renderCullChoiceModal = () => {
    ...
    const canInteract = isMyChoice && canActAs(chooser);

    const selectedCount = pendingTargets.filter((t) => t.kind !== "NONE").length;
    const valid = selectedCount === 1 && pendingTargets[0].kind === "UNIT" && pendingTargets[0].owner === chooser;
    const unitId = valid && pendingTargets[0].kind === "UNIT" ? pendingTargets[0].instanceId : null;
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ fontSize: 18, fontWeight: 900, color: "#fca5a5" }}>Cull the Weak</div>
          <div style={{ marginTop: 8, fontSize: 14, opacity: 0.9 }}>
            {isMyChoice ? "Choose one of your units to sacrifice." : `Waiting for ${chooser} to choose a unit...`}
          </div>
          <div style={{ marginTop: 12, fontSize: 12, opacity: 0.8 }}>Selected: {selectedCount}/1</div>
          {isMyChoice && (
            <div style={{ marginTop: 16, display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button className="rb-miniButton" onClick={() => setPendingTargets([{ kind: "NONE" }])} disabled={!canInteract || selectedCount === 0}>
                Clear
              </button>
              <button className="rb-bigButton" style={{ padding: "8px 16px", fontSize: 14, background: valid ? "#dc2626" : "#334155", cursor: valid ? "pointer" : "not-allowed" }} onClick={() => { if (valid && unitId) { dispatchEngineAction({ type: "CULL_CHOOSE", player: chooser, unitInstanceId: unitId }); setPendingTargets([{ kind: "NONE" }]); } }} disabled={!canInteract || !valid}>
                Confirm Sacrifice
              </button>
            </div>
          )}
      ...
      <div className="rb-grid">
        <div className={`rb-board ${showMulliganUI ? "rb-boardMulligan" : "rb-boardLive"}`}>
          <div className="rb-boardInner">
            ...
            {showMulliganUI ? renderMulliganModal() : null}
          </div >
        </div >
      </div >
    );

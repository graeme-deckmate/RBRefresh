...
type Target =
  ...
  | { kind: "CHAIN_ITEM"; chainItemId: string }  // A spell/ability on the chain (for counter effects)
  | { kind: "NONE" };
...
const getBattlefieldTriggerEffects = (
  ...
  const effects = new Set<string>();
  const rawAll = (
    (bfCard as any)?.rules_text?.raw ||
    (bfCard as any)?.ability?.raw_text ||
    (bfCard as any)?.ability?.effect_text ||
    ""
  ).toString();
  const clauses = extractTriggerClauses(rawAll);
  ...
  if (effects.size === 0) {
    ...
    if (trig && battlefieldTriggerMatchesEvent(trig, event)) {
      ...
      // Direct raw text fallback check for Battlefields
      const { trigger, effectText } = deriveTriggerAndEffect(rawAll, undefined);
      if (trigger && effectText && battlefieldTriggerMatchesEvent(trigger, event)) {
        addEffect(effectText);
      }
    }
  }
...
const checkBecomesMighty = (game: GameState, unit: CardInstance, previousMight: number): void => {
  ...
  if (!wasMighty && nowMighty) {
    ...
    // Also check for "becomes Mighty" triggers on units in play (e.g., Fiora, Worthy)
    const alliedUnits = [
      ...game.players[unit.controller].base.units,
      ...game.battlefields.flatMap((b) => b.units[unit.controller]),
    ];
    for (const ally of alliedUnits) {
      const raw = (ally.ability?.raw_text || "").toLowerCase();
      // Pattern: "when a unit you control becomes [mighty]"
      if (!/when a unit you control becomes \[?mighty\]?/i.test(raw)) continue;
      // Don't double-trigger from the unit itself if it IS the trigger unit
      // (but still queue — the effect may apply to the unit that became Mighty)
      const effectText = ally.ability?.raw_text || "";
      const req = inferTargetRequirement(effectText);
      const chainItem: any = {
        id: makeId("chain"),
        controller: unit.controller,
        kind: "TRIGGERED_ABILITY",
        label: `Trigger: ${ally.name} (${unit.name} became Mighty)`,
        effectText,
        targets: [{ kind: "UNIT", owner: unit.owner, instanceId: unit.instanceId, battlefieldIndex: null, zone: "BF" as const }],
        needsTargets: false, // target is automatically the unit that became Mighty
      ...
      game.priorityPlayer = unit.controller;
      game.passesInRow = 0;
      game.log.unshift(`${ally.name} triggered: ${unit.name} became Mighty.`);
    }
  }
};
...
type TargetRequirement =
  ...
  | { kind: "CHAIN_ITEM"; count: number; optional?: boolean }  // A spell/ability on the chain (counter effects)
  | { kind: "UNIT_FRIENDLY_AND_CHAIN_ITEM"; optional?: boolean };  // Friendly unit + a spell on chain (e.g. Riposte)
...
const inferTargetRequirement = (effectTextRaw: string | undefined, ctx?: { here?: boolean }): TargetRequirement => {
  ...
  // Check for chain/stack targeting — counter spells, Riposte-style
  // "Choose a friendly unit and a spell" → UNIT_FRIENDLY + CHAIN_ITEM
  const wantsFriendlyAndSpell =
    /choose\s+a\s+friendly\s+unit\s+and\s+a\s+spell/i.test(text) ||
    /choose\s+a\s+spell\s+and\s+a\s+friendly\s+unit/i.test(text);
  if (wantsFriendlyAndSpell) return { kind: "UNIT_FRIENDLY_AND_CHAIN_ITEM", optional: isOptional };

  // "Counter target spell" or "counter a spell" alone → CHAIN_ITEM
  const wantsChainOnly =
    /counter\s+target\s+spell/i.test(text) ||
    /counter\s+a\s+spell\s+on\s+the\s+(?:chain|stack)/i.test(text);
  if (wantsChainOnly) return { kind: "CHAIN_ITEM", count: 1, optional: isOptional };
...
const parseAdditionalCostInfo = (effectTextRaw: string): {
  ...
  if (!clause || !/additional\s+cost/i.test(clause)) {
    return { hasAdditionalCost: false, isOptional: false, discardCount: 0, hasBuffCost: false, hasExhaustUnitCost: false, clause: "", clauseLower: "" };
  }
  ...
  const hasBuffCost = clauseLower.includes("spend a buff") || clauseLower.includes("spend any number of buffs");
  const hasExhaustUnitCost = clauseLower.includes("exhaust a friendly unit");
  return { hasAdditionalCost: true, isOptional, discardCount: Math.max(0, discardCount), hasBuffCost, hasExhaustUnitCost, clause, clauseLower };
};
...
const resolveAdditionalCostsForPlay = (
  ...
  if (clauseLower.includes("spend a buff") || clauseLower.includes("spend any number of buffs")) {
    ...
    if (payOptional(canPay) && spendCount > 0) {
      ...
      additionalCostPaid = true;
      // Spending a buff as an additional cost waives the energy cost entirely (alternate payment)
...
const resolveEffectText = (
  ...
  // Chain item target (for counter spells like Riposte)
  const chainItemTarget = rawTargets.find((t): t is Extract<Target, { kind: "CHAIN_ITEM" }> => t.kind === "CHAIN_ITEM") || null;
  const targetedChainItem = chainItemTarget ? game.chain.find((x) => x.id === chainItemTarget.chainItemId) || null : null;

  // Resolve "counter that spell" — remove the targeted chain item from the chain
  if (/counter that spell/i.test(lower) && targetedChainItem) {
    const idx = game.chain.findIndex((x) => x.id === targetedChainItem.id);
    if (idx !== -1) {
      const countered = game.chain.splice(idx, 1)[0];
      game.log.unshift(`${controller} countered ${countered.label || countered.sourceCard?.name || "a spell"}.`);
    }
  }

  // "Give that unit +[S] equal to that spell's Energy cost this turn" (Riposte)
  if (/give that unit \+(?:might|\[s\]|strength) equal to that spell(?:'s)? energy cost/i.test(lower) && unitTarget && targetedChainItem) {
    const spellCost = targetedChainItem.sourceCard?.cost ?? 0;
    if (spellCost > 0) {
      unitTarget.tempMight = (unitTarget.tempMight || 0) + spellCost;
      game.log.unshift(`${controller}: ${unitTarget.name} gains +${spellCost} might (Riposte) this turn.`);
    }
  }
...
const RBEXP = () => {
  ...
  const trySelectTarget = (t: Target): boolean => {
    ...
    // CHAIN_ITEM target — select a spell/ability on the chain
    if (req.kind === "CHAIN_ITEM" && t.kind === "CHAIN_ITEM") {
      setPendingTargets([t]);
      return true;
    }

    // UNIT_FRIENDLY_AND_CHAIN_ITEM — Riposte-style: slot[0]=unit, slot[1]=chain item
    if (req.kind === "UNIT_FRIENDLY_AND_CHAIN_ITEM") {
      if (t.kind === "UNIT" && t.owner === ctx.controller) {
        setPendingTargets([t, pendingTargets[1] || { kind: "NONE" }]);
        return true;
      }
      if (t.kind === "CHAIN_ITEM") {
        // Validate the chain item exists in the chain
        const chainItem = g.chain.find((x) => x.id === t.chainItemId);
        if (!chainItem) return false;
        setPendingTargets([pendingTargets[0] || { kind: "NONE" }, t]);
        return true;
      }
      return false;
  ...
  const queuePlayTriggersForCard = (d: GameState, item: ChainItem) => {
    ...
    // Only gear already in play (base.gear) can be equipped — not gear in hand.
    if (hasKeyword(card, "Weaponmaster") && card.type === "Unit") {
      const p = d.players[item.controller];
      // Only gear already in play (base is in play, hand is not)
      const availableGear = [...p.base.gear];
      ...
      >
        ...
        >
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 2, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#7dd3fc", whiteSpace: "nowrap" as const }}>
              🎯 Choose Target — {item.label}
            </div>
            <div style={{ fontSize: 11, opacity: 0.75, whiteSpace: "nowrap" as const }}>
              Click targets on the board • {selectedCount}/{maxCount} selected
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            <button
              className="rb-miniButton"
              style={canConfirm ? { background: "#0ea5e9", color: "#fff", fontWeight: 700 } : {}}
              onClick={confirmChainChoice}
              disabled={!canConfirm}
            >
              Confirm ({selectedCount}/{maxCount})
            </button>
          </div>
        </div>
      </div>
  ...
  */
  ...
  const arenaCss = `
    ...
    .rb-matRow {
      flex: 1; /* Allow battlefields to expand and fill space */
      overflow: hidden;
      display: grid;
    ...
    .rb-playerRow {
      ...
      flex-direction: row;
      gap: 6px;
      flex: 0 0 148px; /* Fixed height for base panels */
      min-height: 0;
      overflow: hidden;
    }

    .rb-runesCol {
      flex: 0 0 144px; /* Slightly narrower */
      display: flex;
      flex-direction: column;
      gap: 4px;
    ...
    .rb-basePanel {
      flex: 1;
      position: relative;
      border: 2px solid rgba(55,135,255,0.50);
      box-shadow: 0 0 18px rgba(55,135,255,0.14) inset;
      border-radius: 10px;
      padding: 8px;
      overflow: hidden;
      display: flex;
    ...
    .rb-basePanelLabel {
      ...
      font-weight: 900;
      letter-spacing: 8px;
      color: rgba(55,135,255,0.10);
      pointer-events: none;
      user-select: none;
    }

    /* Stagger-stack: cards overlap with negative left margin */
    .rb-cardStack {
      display: flex;
      flex-direction: row;
      flex-wrap: nowrap;
    ...
    .rb-cardStack > * + * { margin-left: -26px; }

    /* Bottom bar: game log + hand + actions */
    .rb-handBar {
      display: flex;
      flex-direction: row;
      gap: 6px;
      flex-shrink: 0;
      align-items: stretch;
      min-height: 180px; /* Taller to accommodate hand + phase bar */
      max-height: 240px;
      overflow: hidden;
    }
  ...
  const ArenaCard = ({
    ...
    return (
      <div
        id={targetId}
        className={cls}
        style={{ opacity: dimmed ? 0.55 : 1, cursor: onClick ? "pointer" : "default", position: "relative" }}
      ...
      >
        ...
        {!facedown && card.type === "Unit" ? (() => {
          const baseMight = (card as any).stats?.might ?? 0;
          const effMight = effectiveMight(card, { role: "NONE", game: g || undefined });
          const mightColor = effMight > baseMight ? "#22c55e" : effMight < baseMight ? "#ef4444" : "#000";
          const hasBuff = ((card as any).buffs ?? 0) > 0;
          const isStunned = (card as any).stunned === true;
          const attachedGear: any[] = (card as any).attachedGear ?? [];
          const tempKws: string[] = (card as any).tempKeywords ?? [];
          return (
            <>
              {/* Might badge - top right, color coded vs base might */}
              <div style={{ position: "absolute", top: 4, right: 4, display: "flex", alignItems: "center", gap: 1 }}>
                {hasBuff && (
                  <div style={{
                    background: "#16a34a", color: "#fff", fontSize: size === "xs" ? 7 : 9,
                    fontWeight: 900, borderRadius: "3px 0 0 3px",
                    padding: size === "xs" ? "1px 2px" : "2px 3px",
                    lineHeight: 1, border: "1px solid rgba(255,255,255,0.3)",
                  }}>B</div>
                )}
                <div style={{
                  background: mightColor, color: "#fff",
                  fontSize: size === "xs" ? 8 : size === "sm" ? 10 : 12,
                  fontWeight: 900, borderRadius: hasBuff ? "0 3px 3px 0" : 3,
                  padding: size === "xs" ? "1px 3px" : "2px 5px",
                  lineHeight: 1, border: "1px solid rgba(255,255,255,0.3)",
                  minWidth: size === "xs" ? 12 : 16, textAlign: "center",
                }}>{effMight}</div>
              </div>

              {/* Damage badge - red circle, only when >0 */}
              {((card as any).damage ?? 0) > 0 && (
                <div style={{
                  position: "absolute", bottom: size === "xs" ? 14 : 20, right: 4,
                  background: "#dc2626", color: "#fff",
                  fontSize: size === "xs" ? 7 : 9, fontWeight: 900,
                  borderRadius: "50%", width: size === "xs" ? 14 : 18, height: size === "xs" ? 14 : 18,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  border: "1.5px solid #fff", lineHeight: 1,
                }}>{(card as any).damage}</div>
              )}
              ...
              {tempKws.length > 0 && (
                <div style={{
                  ...
                  display: "flex", flexWrap: "wrap", gap: 2, pointerEvents: "none",
                }}>
                  {tempKws.slice(0, 3).map((kw, i) => (
                    <div key={i} style={{
                      background: "rgba(139,92,246,0.85)", color: "#fff",
                      fontSize: size === "xs" ? 5 : 7, fontWeight: 700,
                      borderRadius: 2, padding: "1px 2px", lineHeight: 1,
                    }}>{kw}</div>
                  ))}
                </div>
              )}

              {/* Equipment peeking below card */}
              {attachedGear.length > 0 && (
                <div style={{
                  position: "absolute", bottom: -(size === "xs" ? 12 : 18), right: 4,
                  display: "flex", gap: 2,
                }}>
                  {attachedGear.slice(0, 2).map((g2: any, i: number) => (
                    <div
                      key={i}
                      title={g2?.name ?? "Gear"}
                      onMouseEnter={(e) => { e.stopPropagation(); if (g2) setHoverCard(g2 as any); }}
                      onMouseLeave={(e) => { e.stopPropagation(); setHoverCard(null); }}
                      style={{
                        width: size === "xs" ? 16 : 22, height: size === "xs" ? 22 : 30,
                    ...
                    >
                      {g2 && cardImageUrl(g2) ? (
                        <img src={cardImageUrl(g2)} alt={g2.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      ) : (
                        <div style={{ width: "100%", height: "100%", background: "#78350f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 5, color: "#fde68a" }}>⚙</div>
                      )}
                    </div>
                  ))}
                </div>
  ...
  const renderArenaGame = () => {
    ...
    const targetRequirementSatisfied = (req: TargetRequirement, targets: Target[] | null | undefined): boolean => {
      ...
      const paired =
        ...
        req.kind === "UNIT_AND_GEAR_FRIENDLY" ||
        req.kind === "UNIT_AND_GEAR_SAME_CONTROLLER" ||
        req.kind === "UNIT_FRIENDLY_AND_CHAIN_ITEM";
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 8 }}>
              {pendingAdditionalCostInfo.hasAdditionalCost && pendingAdditionalCostInfo.isOptional ? (
                <label className="rb-softText" style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                  ...
                  />
                  {pendingAdditionalCostInfo.hasBuffCost
                    ? (() => {
                      const allUnits = pendingPlay
                        ? [
                          ...g.players[pendingPlay.player].base.units,
                          ...g.battlefields.flatMap((b) => b.units[pendingPlay.player]),
                        ]
                        : [];
                      const totalBuffs = allUnits.reduce((s, u) => s + (u.buffs || 0), 0);
                      return (
                        <span>
                          Pay with buff{totalBuffs > 0 ? ` (${totalBuffs} buff${totalBuffs !== 1 ? "s" : ""} available ✓)` : " — no buffs available ✗"}
                        </span>
                      );
                    })()
                    : pendingAdditionalCostInfo.hasExhaustUnitCost
                      ? "Pay optional cost: exhaust a friendly unit"
                      : "Pay optional additional cost"}
                </label>
              ) : null}

              {pendingAdditionalCostInfo.hasAdditionalCost && !pendingAdditionalCostInfo.isOptional && pendingAdditionalCostInfo.hasBuffCost ? (
                (() => {
                  const allUnits = pendingPlay
                    ? [
                      ...g.players[pendingPlay.player].base.units,
                  ...
                  const totalBuffs = allUnits.reduce((s, u) => s + (u.buffs || 0), 0);
                  return (
                    <div className="rb-softText" style={{ color: totalBuffs > 0 ? "#4ade80" : "#f87171" }}>
                      Required: spend a buff ({totalBuffs > 0 ? `${totalBuffs} available ✓` : "none available ✗"})
                    </div>
                  );
                })()
              ) : null}
                  ...
                  return (
                    <ArenaCard
                      ...
                      onClick={() => {
                        if (!clickable) return;
                        setArenaMove((s) => {
                          const thisFrom = { kind: "BF" as const, index: idx };
                          // If no move started, start with this unit's BF as source
                          if (!s) return { from: thisFrom, unitIds: [u.instanceId] };

                          // Toggle: if already selected, remove from selection
                          if (s.unitIds.includes(u.instanceId)) {
                            const nextIds = s.unitIds.filter((id) => id !== u.instanceId);
                            return nextIds.length === 0 ? null : { ...s, unitIds: nextIds };
                          }

                          // Same BF as current source: add to selection
                          if (s.from.kind === "BF" && s.from.index === idx) {
                            return { ...s, unitIds: [...s.unitIds, u.instanceId] };
                          }

                          // Different BF: allow adding units with Ganking (multi-source)
                          // We store the move as "from: MULTI" by keeping the first BF as logical
                          // source; executeStandardMoveWith handles each unit's from separately
                          const hasGanking = hasKeyword(u, "Ganking");
                          if (hasGanking) {
                            return { ...s, unitIds: [...s.unitIds, u.instanceId] };
                          }

                          // Non-ganking unit on different BF: reset to this unit only
                          return { from: thisFrom, unitIds: [u.instanceId] };
                        });
                      }}
                      onDoubleClick={() => setHoverCard(u)}
                    />
                  );
                })}  {/* end rb-row for me units */}
              </div>
            ...
            {ps.base.gear.length > 0 && (
              <div className="rb-row rb-rowTight" style={{ marginTop: 8 }}>
                {ps.base.gear.map((gear) => {
                  ...
                  // WEAPONMASTER: check if this gear can be selected for equip
                  const wmc = g.pendingWeaponmasterChoice;
                  const isWeaponmasterTarget = isMe && wmc && wmc.unitOwner === me && wmc.availableGearIds.includes(gear.instanceId);
                  ...
                  return (
                    <div key={gear.instanceId} style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "center" }}>
                      <ArenaCard
                        ...
                        showReadyDot={true}
                        style={{
                          outline: isWeaponmasterTarget ? "2px solid #f59e0b" : undefined,
                          borderRadius: 8,
                          cursor: isWeaponmasterTarget ? "pointer" : undefined,
                        } as React.CSSProperties}
                        onClick={() => {
                          // WEAPONMASTER: click-to-equip
                          if (isWeaponmasterTarget && wmc) {
                            updateGame((d) => {
                              const choice = d.pendingWeaponmasterChoice;
                              if (!choice) return;
                              const loc = locateUnit(d, choice.unitOwner, choice.unitInstanceId);
                              if (!loc) { d.pendingWeaponmasterChoice = null; return; }
                              const unit = loc.unit;
                              const baseIdx = d.players[me].base.gear.findIndex((g2) => g2.instanceId === gear.instanceId);
                              if (baseIdx < 0) { d.pendingWeaponmasterChoice = null; return; }
                              const realGear = d.players[me].base.gear.splice(baseIdx, 1)[0];
                              if (!unit.attachedGear) unit.attachedGear = [];
                              unit.attachedGear.push(realGear);
                              d.log.unshift(`${realGear.name} equipped to ${unit.name} (Weaponmaster).`);
                              d.pendingWeaponmasterChoice = null;
                            });
                            return;
                          }
                          setHoverCard(gear as any);
                        }}
                        onDoubleClick={() => {
                          if (canEquip) {
        ...
        <div>
          ...
          <div className="rb-actionHint">
            {g.pendingCandlelitChoice && g.pendingCandlelitChoice.player === me ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {(() => {
                  ...
                  const setChoice = (cardId: string, next: "KEEP" | "RECYCLE") => {
                    // Use updateGame to operate on the REAL game state, not the projected stub view
                    updateGame((d) => {
                      if (!d.pendingCandlelitChoice) return;
                      d.pendingCandlelitChoice.choices = { ...(d.pendingCandlelitChoice.choices || {}), [cardId]: next };
                      if (next === "RECYCLE" && d.pendingCandlelitChoice.order) {
                        d.pendingCandlelitChoice.order = d.pendingCandlelitChoice.order.filter((id) => id !== cardId);
                      }
                    });
                  };
                  const setTop = (cardId: string) => {
                    updateGame((d) => {
                      if (!d.pendingCandlelitChoice) return;
                      const currentKept = d.pendingCandlelitChoice.cards
                        .filter((c) => (d.pendingCandlelitChoice!.choices?.[c.instanceId] || "KEEP") !== "RECYCLE")
                        .map((c) => c.instanceId);
                      d.pendingCandlelitChoice.order = [cardId, ...currentKept.filter((id) => id !== cardId)];
                    });
                  };
                  return (
                    <>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <button
                          className="rb-miniButton"
                          onClick={() => {
                            // Snapshot choice data from the projected view (choices/order are safe — they're just IDs)
                            const pending = g.pendingCandlelitChoice!;
                            const map = pending.choices || {};
                            const keptIds = pending.cards.filter((c) => (map[c.instanceId] || "KEEP") !== "RECYCLE").map((c) => c.instanceId);
                            const recycledIds = pending.cards.filter((c) => (map[c.instanceId] || "KEEP") === "RECYCLE").map((c) => c.instanceId);
                            const orderIds = (pending.order || []).filter((id) => keptIds.includes(id));
                            for (const id of keptIds) { if (!orderIds.includes(id)) orderIds.push(id); }
                            const player = pending.player;
                            // Apply to REAL game state via updateGame
                            updateGame((d) => {
                              const dp = d.players[player];
                              // The real cards are in d.pendingCandlelitChoice.cards (not stubs)
                              const realCards = d.pendingCandlelitChoice?.cards ?? [];
                              const orderedKept = orderIds.map((id) => realCards.find((c) => c.instanceId === id)).filter((c): c is CardInstance => !!c);
                              const recycledCards = recycledIds.map((id) => realCards.find((c) => c.instanceId === id)).filter((c): c is CardInstance => !!c);
                              if (orderedKept.length > 0) {
                                dp.mainDeck = orderedKept.concat(dp.mainDeck);
                              }
                              if (recycledCards.length > 0) {
                                dp.mainDeck.push(...recycledCards);
                              }
                              d.log.unshift(`${player} resolved Candlelit Sanctum (recycled ${recycledCards.length}).`);
                              d.pendingCandlelitChoice = null;
                            });
                          }}
                        >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <div><b>Weaponmaster:</b> Click a gear card in your base to equip it (or skip).</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {(() => {
                    const choice = g.pendingWeaponmasterChoice!;
                    return (
                      <>
                        <span className="rb-softText" style={{ fontSize: 11 }}>
                          Gear in play is highlighted — click it to equip.
                        </span>
                        <button
                          className="rb-miniButton"
                          onClick={() => {
      ...
      <div className="rb-grid">
        <div className={`rb-board ${showMulliganUI ? "rb-boardMulligan" : "rb-boardLive"}`}>
          <div className="rb-boardInner">
            ...
            {/* ═══════════════════════════════════════════════════
                OPP HAND STRIP — thin row: facedown cards + compact legend/champion + rune deck
            ═══════════════════════════════════════════════════ */}
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0, minHeight: 64, overflow: "hidden" }}>
              <div style={{ flex: 1, minWidth: 0 }}>{renderTopLane()}</div>
              {/* Compact opp legend + champion */}
              {(() => {
                const ps = oppState;
                const legendCard = ps.legend ? ({
                  ...(ps.legend as any), instanceId: `legend_${opp}`, owner: opp, controller: opp,
                  isReady: ps.legendReady, damage: 0, buffs: 0, tempMightBonus: 0, stunned: false, createdTurn: 0,
                } as CardInstance) : null;
                return (
                  <div style={{ display: "flex", gap: 4, alignItems: "center", flexShrink: 0 }}>
                    {legendCard && <ArenaCard card={legendCard} size="sm" showReadyDot={false} onClick={() => setHoverCard(ps.legend as any)} />}
                    {ps.championZone && <ArenaCard card={ps.championZone} size="sm" showReadyDot={false} onClick={() => setHoverCard(ps.championZone as any)} />}
                    <div style={{ opacity: ps.runeDeck.length > 0 ? 1 : 0.35, flexShrink: 0 }}>
                      <ArenaCard card={zoneBackCard(opp, "Rune Deck")} facedown size="sm" showReadyDot={false} />
                    </div>
                  </div>
                );
              })()}
            ...
            {/* ═══════════════════════════════════════════════════
                OPP PLAYER ROW: rune column (left) | BASE panel (right)
            ═══════════════════════════════════════════════════ */}
            <div className="rb-playerRow">
              {/* Opp runes column */}
              <div className="rb-runesCol">
                <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 2 }}>Runes</div>
                {oppState.runesInPlay.length === 0 ? <span className="rb-softText" style={{ fontSize: 11, textAlign: "center" }}>No Runes</span> : null}
                <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                  {oppState.runesInPlay.map((r) => {
                    const img = cardImageUrl(r);
                    return (
                      <div key={r.instanceId} className={`rb-rune ${!r.isReady ? "rb-runeExhausted" : ""}`} onClick={() => setHoverCard(r as any)} title={`${r.name || "Rune"} (${r.domain})`} style={{ flexShrink: 0 }}>
                        {img ? <img src={img} alt={r.name || r.domain} /> : null}
                      </div>
                    );
                  })}
              ...
              {/* Opp BASE panel */}
              <div className="rb-basePanel">
                <div className="rb-basePanelLabel">BASE</div>
                {/* Opp base units */}
                <div style={{ display: "flex", flexDirection: "column", gap: 4, position: "relative", zIndex: 1 }}>
                  <div className="rb-cardStack">
                    {oppState.base.units.map((u) => (
                      <ArenaCard key={u.instanceId} targetId={`board-target-${u.instanceId}`} card={{ ...(u as any) }} size="sm" showReadyDot={true} onClick={() => setHoverCard(u as any)} />
                    ))}
                  </div>
                  <div className="rb-cardStack">
                    {oppState.base.gear.map((gear) => (
                      <ArenaCard key={gear.instanceId} targetId={`board-target-${gear.instanceId}`} card={{ ...(gear as any) }} size="sm" showReadyDot={true} onClick={() => setHoverCard(gear as any)} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            {/* ═══════════════════════════════════════════════════
                BATTLEFIELDS — narrow center strip
            ═══════════════════════════════════════════════════ */}
            ...
            {/* ═══════════════════════════════════════════════════
                MY PLAYER ROW: rune column (left) | BASE panel (right, with chain)
            ═══════════════════════════════════════════════════ */}
            <div className="rb-playerRow">
              {/* My runes column — interactive */}
              {(() => {
                const ps = meState;
                return (
                  <div className="rb-runesCol">
                    <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 2 }}>Runes</div>
                    <div style={{ opacity: ps.runeDeck.length > 0 ? 1 : 0.35, flexShrink: 0, marginBottom: 2 }}>
                      <ArenaCard card={zoneBackCard(me, "Rune Deck")} facedown size="sm" showReadyDot={false} />
                    </div>
                    {ps.runesInPlay.length === 0 ? <span className="rb-softText" style={{ fontSize: 11, textAlign: "center" }}>No Runes</span> : null}
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                      {ps.runesInPlay.map((r) => {
                        const h = hoverPayPlan?.plan.runeUses[r.instanceId];
                        const img = cardImageUrl(r);
                        const cls = ["rb-rune", !r.isReady ? "rb-runeExhausted" : "", h === "EXHAUST" ? "rb-runeGlowExhaust" : "", h === "RECYCLE" ? "rb-runeGlowRecycle" : "", h === "BOTH" ? "rb-runeGlowBoth" : ""].filter(Boolean).join(" ");
                        return (
                          <div key={r.instanceId} className={cls} onClick={() => exhaustRuneForEnergy(me, r.instanceId)} onContextMenu={(e) => { e.preventDefault(); recycleRuneForPower(me, r.instanceId); }} title={`${r.name || "Rune"} (${r.domain}) • L-Click: +1E • R-Click: +1P`} style={{ flexShrink: 0 }}>
                            {img ? <img src={img} alt={r.name || r.domain} /> : null}
                          </div>
                        );
                      })}
                    </div>
                    {isMyTurn && <div style={{ fontSize: 9, opacity: 0.5, marginTop: 2 }}>L-click: +1E • R-click: +1P</div>}
                  </div>
                );
              })()}
              {/* My BASE panel — click target for move-to-base + chain inline */}
              <div
                className="rb-basePanel"
                style={{
                  border: arenaMove && canSelectMoveUnits ? "2px dashed rgba(130, 210, 255, 0.60)" : undefined,
                  cursor: arenaMove && canSelectMoveUnits && arenaMove.from.kind !== "BASE" ? "pointer" : undefined,
                }}
                onClick={() => {
              ...
              >
                <div className="rb-basePanelLabel">BASE</div>
                {/* My base units + gear stagger-stacked */}
                <div style={{ display: "flex", flexDirection: "column", gap: 4, position: "relative", zIndex: 1, flex: 1 }}>
                  {arenaMove && canSelectMoveUnits ? (
                    <div className="rb-actionHint" style={{ fontSize: 11, marginBottom: 4 }}>
                      Move armed — click a BF or here to move.{" "}
                      <button className="rb-miniButton" onClick={(e) => { e.stopPropagation(); setArenaMove(null); }}>Cancel</button>
                    </div>
                  ) : null}
                  <div className="rb-cardStack">
                    {meState.base.units.map((u) => {
                      const wmc = g.pendingWeaponmasterChoice;
                      const isWmcUnit = wmc && wmc.unitInstanceId === u.instanceId;
                      return (
                        <div key={u.instanceId} style={{ display: "flex", flexDirection: "column", gap: 2, alignItems: "center" }}>
                          <ArenaCard
                            targetId={`board-target-${u.instanceId}`}
                            card={{ ...(u as any) }}
                            size="sm"
                            showReadyDot={true}
                            selected={arenaMove?.unitIds.includes(u.instanceId) ?? false}
                            style={isWmcUnit ? { outline: "2px solid #22d3ee", borderRadius: 8 } as React.CSSProperties : undefined}
                            onClick={() => {
                              if (arenaMove?.from.kind === "BASE" && arenaMove.unitIds.includes(u.instanceId)) {
                                setArenaMove((m) => ({ ...m!, unitIds: m!.unitIds.filter((id) => id !== u.instanceId) }));
                              } else if (u.isReady && canActAs(me)) {
                                setArenaMove({ from: { kind: "BASE" }, unitIds: [u.instanceId] });
                              } else {
                                setHoverCard(u as any);
                              }
                          ...
                          />
                          {arenaMove?.from.kind === "BASE" && arenaMove.unitIds.includes(u.instanceId) ? (
                            <div style={{ fontSize: 9, color: "var(--cp-neon-cyan)" }}>[MOVING]</div>
                          ) : null}
                        </div>
                      );
                    })}
                  </div>
                  {/* My base gear rendered here is handled by BaseRow for Weaponmaster interaction */}
                  {(() => {
                    const ps = meState;
                    const canLocalAct = canActAs(me) && g.priorityPlayer === me;
                    return ps.base.gear.length > 0 ? (
                      <div className="rb-cardStack" style={{ marginTop: 4 }}>
                        {ps.base.gear.map((gear) => {
                          const canEquip = canLocalAct && isEquipment(gear) && !!parseEquipCost(gear);
                          const canActivateGear = canLocalAct && gear.isReady && !!gearActivatedEffect(gear);
                          const canSpendSeal = canLocalAct && gear.isReady && !!getSealPowerDomain(gear, ps.domains);
                          const wmc = g.pendingWeaponmasterChoice;
                          const isWeaponmasterTarget = wmc && wmc.unitOwner === me && wmc.availableGearIds.includes(gear.instanceId);
                          return (
                            <div key={gear.instanceId} style={{ display: "flex", flexDirection: "column", gap: 2, alignItems: "center" }}>
                              <ArenaCard
                                targetId={`board-target-${gear.instanceId}`}
                                card={{ ...(gear as any) }}
                                size="sm"
                                showReadyDot={true}
                                style={isWeaponmasterTarget ? { outline: "2px solid #f59e0b", borderRadius: 8, cursor: "pointer" } as React.CSSProperties : undefined}
                                onClick={() => {
                                  if (isWeaponmasterTarget && wmc) {
                                    updateGame((d) => {
                                      const choice = d.pendingWeaponmasterChoice;
                                ...
                                onDoubleClick={() => {
                                  if (canEquip) { dispatchEngineAction({ type: "EQUIP_START", player: me, gearInstanceId: gear.instanceId }); return; }
                                  if (canActivateGear) { dispatchEngineAction({ type: "GEAR_ACTIVATE", player: me, gearInstanceId: gear.instanceId, autoPay: autoPayEnabled }); }
                                }}
                              />
                              <div style={{ display: "flex", gap: 2, flexWrap: "wrap", justifyContent: "center" }}>
                                {canEquip && <button className="rb-miniButton" style={{ fontSize: 8 }} onClick={() => dispatchEngineAction({ type: "EQUIP_START", player: me, gearInstanceId: gear.instanceId })}>Equip</button>}
                                {canActivateGear && <button className="rb-miniButton" style={{ fontSize: 8 }} onClick={() => dispatchEngineAction({ type: "GEAR_ACTIVATE", player: me, gearInstanceId: gear.instanceId, autoPay: autoPayEnabled })}>Activate</button>}
                                {canSpendSeal && <button className="rb-miniButton" style={{ fontSize: 8 }} onClick={() => dispatchEngineAction({ type: "SEAL_EXHAUST", player: me, gearInstanceId: gear.instanceId })}>Seal +1P</button>}
                              </div>
                            </div>
                          );
                        })}
                ...
                {/* CHAIN — inline on the right of my base panel */}
                {g.chain.length > 0 && (
                  <div className="rb-chainPanel">
                    <div style={{ fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, textAlign: "center", marginBottom: 4 }}>Chain ({g.chain.length})</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {g.chain.map((item, chainIdx) => {
                        const isTop = chainIdx === 0;
                        const targetNames = (item.targets || []).filter((t): t is { kind: "UNIT"; owner: PlayerId; instanceId: string } => t.kind === "UNIT").map((t) => locateUnit(g, t.owner, t.instanceId)?.unit.name || "?");
                        let cardObj = item.sourceCard || null;
                        if (!cardObj && item.sourceInstanceId) {
                          const pool = [...g.battlefields.flatMap(bf => [...bf.units["P1"], ...bf.units["P2"], ...bf.gear["P1"], ...bf.gear["P2"]]), ...g.players["P1"].base.units, ...g.players["P2"].base.units, ...g.players["P1"].base.gear, ...g.players["P2"].base.gear, ...g.players["P1"].hand, ...g.players["P2"].hand, g.players["P1"].championZone, g.players["P2"].championZone].filter(Boolean);
                          cardObj = (pool.find(c => c && c.instanceId === item.sourceInstanceId) as CardInstance) || null;
                        }
                        return (
                          <div
                            key={item.id}
                            ...
                            style={{
                              display: "flex", flexDirection: "column", alignItems: "center", padding: 0, background: "transparent", border: "none",
                              outline: (targetConfirmReq.kind === "CHAIN_ITEM" || targetConfirmReq.kind === "UNIT_FRIENDLY_AND_CHAIN_ITEM") ? "2px solid #f59e0b" : undefined,
                              borderRadius: 8,
                              cursor: (targetConfirmReq.kind === "CHAIN_ITEM" || targetConfirmReq.kind === "UNIT_FRIENDLY_AND_CHAIN_ITEM") ? "pointer" : undefined,
                            }}
                            onClick={() => {
                              if (targetConfirmReq.kind === "CHAIN_ITEM" || targetConfirmReq.kind === "UNIT_FRIENDLY_AND_CHAIN_ITEM") trySelectTarget({ kind: "CHAIN_ITEM", chainItemId: item.id });
                          ...
                          >
                            {cardObj ? (
                              <div style={{ position: "relative" }}>
                                ...
                                {isTop && <div style={{ position: "absolute", top: -8, right: -8, background: "#ef4444", color: "white", fontSize: 9, padding: "1px 5px", borderRadius: 4, fontWeight: "bold" }}>TOP</div>}
                              </div>
                            ) : (
                              <div className="rb-chainItemLabel" style={{ textAlign: "center", width: "100%", padding: "10px 6px", background: "#1e293b", borderRadius: 8, fontSize: 11 }}>
                                {isTop ? "▶ " : ""}{item.label}
                              </div>
                            )}
                            {targetNames.length > 0 && <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>🎯 {targetNames.join(", ")}</div>}
                            {item.needsTargets && <div style={{ fontSize: 10, color: "#f97316", marginTop: 2 }}>⏳ Target needed</div>}
                          </div>
                        );
                      })}
                    </div>
            ...
            {/* ═══════════════════════════════════════════════════
                BOTTOM BAR: game log | hand + actions | compact legend/champion
            ═══════════════════════════════════════════════════ */}
            <div className="rb-handBar">
              {/* Game log — left */}
              <div className="rb-logPanel">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div style={{ fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: 1 }}>Game Log</div>
                  <button className="rb-miniButton" onClick={() => updateGame((d) => (d.log = []))}>Clear</button>
                </div>
                <div style={{ flex: 1, overflowY: "auto", fontSize: 11, display: "flex", flexDirection: "column-reverse" }}>
              ...
              {/* My compact legend + activate + champion */}
              {(() => {
                const ps = meState;
                const interactiveLegend = canActAs(me) && g.priorityPlayer === me;
                const legendCard = ps.legend ? ({
                  ...(ps.legend as any), instanceId: `legend_${me}`, owner: me, controller: me,
                  isReady: ps.legendReady, damage: 0, buffs: 0, tempMightBonus: 0, stunned: false, createdTurn: 0,
                } as CardInstance) : null;
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "center", flexShrink: 0 }}>
                    {legendCard && (
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                        <ArenaCard card={legendCard} size="sm" showReadyDot={true} onClick={() => setHoverCard(ps.legend as any)} />
                        {interactiveLegend && (
                          <button className="rb-miniButton" style={{ fontSize: 9 }} disabled={!ps.legendReady || !legendActivationStatus?.ok} onClick={() => dispatchEngineAction({ type: "LEGEND_ACTIVATE", player: me, autoPay: autoPayEnabled })} title={!legendActivationStatus?.ok ? legendActivationStatus?.reason : "Activate Legend"}>Activate</button>
                        )}
                      </div>
                    )}
                    {ps.championZone && (
                      <ArenaCard card={ps.championZone} size="sm" showReadyDot={false} onClick={() => { if (interactiveLegend) beginPlayChampion(me); else setHoverCard(ps.championZone as any); }} onDoubleClick={() => { if (interactiveLegend) beginPlayChampion(me); }} />
                    )}
                  </div>
                );
              })()}
              ...
              {hoverCard ? (
                <div style={{ marginTop: 12 }}>
                  ...
                  {"instanceId" in (hoverCard as any) ? (
                    <div className="rb-row rb-rowCenter">
                      <ArenaCard card={hoverCard as any} size="md" showReadyDot={false} />
                    </div>
                  ) : null}
                  <div className="rb-softText" style={{ marginTop: 8 }}>
